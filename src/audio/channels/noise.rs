...
impl Channel for Noise {
    ...
    fn length_clock(&mut self) {
        if self.enabled && test_bit!(self.trigger_reg, 6) {
...
impl Noise {
    fn trigger(&mut self) {
        ...
        const FREQ_SHIFT_MASK: u8 = bits![7, 6, 5, 4];
        const FREQ_DIVISOR_MASK: u8 = bits![2, 1, 0];
    ...
    fn lfsr_step(&mut self) {
        ...
        if test_bit!(self.poly_counter_reg, 3) {
